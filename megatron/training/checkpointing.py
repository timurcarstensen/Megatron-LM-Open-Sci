...
def save_checkpoint(
    ...
    if (
        ...
        if ckpt_type != CheckpointType.LEGACY and args.use_distributed_optimizer:
            ...
            print_rank_0(
                f"Storing distributed optimizer sharded state of type {optim_sd_kwargs['sharding_type']}"
            )
        ...
        if ckpt_type == CheckpointType.GLOBAL and ckpt_format == "torch_dist":
            ...
            if (
                ...
                and "save_strategy" in checkpointing_context
            ):
                ...
                if (
                    ...
                    save_strategy.use_cached_ckpt_structure = (
                        args.ckpt_assume_constant_structure
                    )
                    if (
                        checkpointing_context is not None
                        ...
                        cached_global_metadata = getattr(
                            checkpointing_context["load_strategy"],
                            "cached_global_metadata",
                            None,
                        )
                        if cached_global_metadata is not None:
                            logger.debug(
                                "Plugging in the read metadata from the load strategy..."
                            )
                            save_strategy.cached_global_metadata = (
                                cached_global_metadata
                            )
                        else:
                            logger.debug(
            ...
            if has_nvidia_modelopt:
                save_sharded_modelopt_state(
                    model, checkpoint_name, (args.ckpt_format, 1)
                )
            ...
            if has_nvidia_modelopt:
                if ckpt_type == CheckpointType.LOCAL:
                    print_rank_0(
                        "WARNING: Local checkpointing does not support nvidia_modelopt."
                    )
                else:
            ...
            if ckpt_type == CheckpointType.LOCAL:
                ...
                async_save_request = checkpointing_context[
                    "local_checkpoint_manager"
                ].save(state_dict_for_save, iteration, is_async=bool(args.async_save))
    ...
    if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
        ...
        if ckpt_type == CheckpointType.LOCAL:
            ...
            def iter_finalize_fn():
                print_rank_0(
                    "  successfully saved local checkpoint from iteration {:7d}".format(
                        iteration
                    )
                )
            ...
            def iter_finalize_fn():
                ...
                print_rank_0(
                    f"  successfully saved checkpoint from iteration {int(iteration):7d} to {args.save} "
    ...
    if args.async_save:
        ...
        print_rank_0(
            "  scheduled an async checkpoint save at iteration {:7d} to {}".format(
                iteration, save_dir
            )
        )
...
def cleanup_old_non_persistent_checkpoint(save_dir, leave_ckpt_num=1, do_async=False):
    ...
    print_rank_0(
        f"Non-persistent checkpoints to be kept: {sorted_iter_ckpts[-leave_ckpt_num:]}"
    )
...
def maybe_save_dataloader_state(train_iterator, iteration, dataloader_save_path):
    ...
    if (
        train_iterator is None
        or dataloader_save_path is None
        or dataloader_save_path == ""
    ):
        return
    ...
    print(
        f"saving dataloader checkpoint at iteration {iteration} to {dataloader_save_path}"
    )
...
def _get_non_persistent_iteration(
    non_persistent_global_dir, args, checkpointing_context=None
):
    if args.non_persistent_ckpt_type is None:
        ...
        if os.path.isfile(tracker_filename):
            ...
            if release:
                raise RuntimeError(
                    "Non-persistent checkpoint can't be a release checkpoint"
                )
        else:
            iteration = -1
            print_rank_0(
...
def _load_global_dist_base_checkpoint(
    ...
    checkpoint_name = get_checkpoint_name(
        load_dir, iteration, release, return_base_dir=True
    )
...
def _get_checkpoint_format(checkpoint_name):
    ...
    is_torch_ckpt = any(
        [f.startswith("mp_rank_0") for f in os.listdir(checkpoint_name)]
    )
...
def _load_base_checkpoint(
    ...
    if non_persistent_iteration != -1:  # there is a non-persistent checkpoint
        if non_persistent_iteration >= iteration:
            ...
            print_rank_0(
                "WARNING: non-persistent checkpoints are older than persistent checkpoint"
            )
...
def load_checkpoint(
    ...
    if pretrained_dir is not None and not checkpoint_exists(load_dir):
        ...
        if not checkpoint_exists(load_dir):
            raise FileNotFoundError(
                "No checkpoint found in load directory or pretrained directory"
            )
        args.finetune = True
    ...
    if ckpt_format == "torch_dist":
        ...
        if (
            ...
            if args.use_distributed_optimizer:
                ...
                if (
                    ckpt_tp_pp != run_tp_pp
                    and optim_sd_kwargs["sharding_type"] != "fully_sharded_model_space"
                ):
                    raise RuntimeError(
    ...
    if args.finetune or release:
        ...
        try:
            ...
            try:  # Backward compatible with older checkpoints
                ...
                sys.exit()
    num_floating_point_operations_so_far = state_dict.get(
        "num_floating_point_operations_so_far", 0
    )
    ...
    if "args" in state_dict and not args.finetune:
        ...
        args.skipped_train_samples = getattr(
            checkpoint_args, "skipped_train_samples", 0
        )
        update_num_microbatches(
            consumed_samples=args.consumed_train_samples, verbose=True
        )
